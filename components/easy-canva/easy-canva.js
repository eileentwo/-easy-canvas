...
Component({
  ...
  methods: {
    ...
    drawRound(params) {
      ...
      num = (2 * Math.PI / 100 *percent) - 0.5 * Math.PI;

      this.ctx.beginPath();
      this.ctx.arc( left, top, width, 0, 2 * Math.PI, false );
      this.ctx.lineTo( left,top );
      this.ctx.setFillStyle(background);
      this.ctx.fill();
      this.ctx.closePath();

      this.ctx.arc(left, top, width-border, -0.5 * Math.PI, num); //绘制的动作
      this.ctx.setStrokeStyle(borderColor); //圆环线条的颜色
      this.ctx.setLineWidth(border);	//圆环的粗细
